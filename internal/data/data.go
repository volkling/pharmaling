...
// InsertRelation adds a non-unique relation between two entries and automatically adds the reverse relation.
func (d *Data) InsertRelation(fromID, toID int, r Relation) (int, error) {
    // Insert the original relation
    ...
    _, err = d.insertRelation(toID, fromID, r, d.queries.InsertRelation)
    if err != nil {
        return 0, err
    }
